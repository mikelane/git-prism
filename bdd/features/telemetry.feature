...
Feature: OpenTelemetry metrics and traces for the MCP server
  ...
  @ISSUE-47 @ISSUE-48 @not_implemented
  ...
  Scenario: Ref patterns are normalized to bounded enum values
    ...
    When I start "git-prism serve" against that repo and call "get_change_manifest"
    And I wait 5 seconds for any exports
    Then every "ref.base" span attribute value is in {"worktree", "single_commit", "range_double_dot", "range_triple_dot", "branch", "sha"}
    And every "ref.head" span attribute value is in {"worktree", "single_commit", "range_double_dot", "range_triple_dot", "branch", "sha"}
